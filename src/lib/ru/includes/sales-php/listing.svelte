...
<div class="row no-gutters" id="MyDetails">
    <div class="col-lg-5 col-md-6 DetailsBox">
        <a href={baseurl+"/de/dubai/off-plan-projects/"+slug} target="_blank">
            <Swiper lazy={true} rewind={true} navigation={true} pagination={{ dynamicBullets: true, }} modules={[Lazy,Pagination,Navigation]}  class="mySwiper">
                ...
                -->
                <SwiperSlide>
                    <img data-src="https://dev.drivenproperties.ru/theme/assets/images/community/{images}" src="{LazyLoadImgURL}" class="swiper-lazy" alt={title }/>
                </SwiperSlide>
            </Swiper>
        </a>
    </div>
    ...
    <div class="col-lg-7  col-md-6 ">
        <div class="card-body">
            <h5 class="card-title HPrice"><a href={baseurl+"/de/dubai/off-plan-projects/"+slug} target="_blank">{@html title }</a></h5>
            ...
            <div class="card-text d-flex flex-row bd-highlight Bxinfo">
                {#if price}
                    <h3 class="HPrice">{CurrancySBC(parseInt(CsPrice *.25))} EUR</h3>
                {:else}
                    <h5 class="mt-1 mb-4">Preis auf Anfrage</h5>
                {/if}
            </div>
...
<style>
    ...
    @media only screen and (max-width: 768px) {
        ...
        #MyDetails a{  color: #fff; margin: 0px 6px 0px 0px; }
        #MyDetails a.telephone {background-color:#263977;}
        #MyDetails a.whatsapp {background-color:#22D672;}
