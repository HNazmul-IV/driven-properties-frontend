            ...
            <div id="SimilarProperties">
                ...
                <div class="g-0 row mb-3 mt-3">
                    {#each SimilarProperties as MySimilarPro }
                        <a href="/ru/{(MySimilarPro.Emirate.toLowerCase())}/{(MySimilarPro.Unit_Type.toLowerCase())}-for-{(MySimilarPro.Ad_Type.toLowerCase())}/{(MySimilarPro.Community.toLowerCase()).replaceAll(' ','-')}/{(MySimilarPro.Property_Name.toLowerCase()).replaceAll(' ','-')}/{MySimilarPro.Property_Ref_No.toLowerCase()}" 
                           class="">
                            <div class="col-sm-12">
                                ...
                                <p><strong>{CurrancySBC(MySimilarPro.Price *.27)} USD</strong></p>
                            </div>
                        </a>
                    {/each}
                </div>
