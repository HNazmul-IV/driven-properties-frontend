<script>
    ...
    const getImages = () => {
        // console.log('fdsfsdfsdfsdfsdfsdfdfdfsdfsdfsdfdsfsdfsdfsdfsdfsdfsdfsdfsdf');
        // console.log(images);
        // setTimeout(()=> {
...
<div class="row no-gutters" id="InnerMyDetails">
    ...
    <div class="col-lg-5 col-md-6 DetailsBox">
        <Swiper lazy={true} rewind={true} navigation={true} pagination={{ dynamicBullets: true, }} modules={[Lazy,Pagination,Navigation]}  class="mySwiper">
            ...
            {#if images.image}
            <SwiperSlide>
                <a href="/ru/{(Emirate).toLowerCase()}/{(building).toLowerCase()}-for-{(PropertyStatus).toLowerCase()}/{((Community).toLowerCase()).replaceAll(' ','-')}/{((propertyTitle).toLowerCase()).replaceAll(' ','-')}/{(objectID).toLowerCase()}" id="MyLksImg" >
                    <img data-src="{images.image[0]}" src="{images.image[0]}" class="swiper-lazy img-fluid" alt={title}/>
                </a>
            </SwiperSlide>
            {:else}
                <div>
                    <a href="/ru/{(Emirate).toLowerCase()}/{(building).toLowerCase()}-for-{(PropertyStatus).toLowerCase()}/{((Community).toLowerCase()).replaceAll(' ','-')}/{((propertyTitle).toLowerCase()).replaceAll(' ','-')}/{(objectID).toLowerCase()}" id="MyLksImg" >
                        <div class="no-image"><em>NO IMAGE FOUND!</em></div>
                    </a>
                </div>
            {/if}
        </Swiper>
    ...
    <div class="col-lg-7  col-md-6">
        <div class="card-body">
            ...
            <h5 class="card-title HPrice">
                <a href="/ru/{(Emirate).toLowerCase()}/{(building).toLowerCase()}-for-{(PropertyStatus).toLowerCase()}/{((Community).toLowerCase()).replaceAll(' ','-')}/{((propertyTitle).toLowerCase()).replaceAll(' ','-')}/{(objectID).toLowerCase()}" class="MyLkstxt">
                    {@html title }
                </a>
            </h5>
            ...
            <div class="card-text d-flex flex-row bd-highlight">
                ...
                <div class="p-0 bd-highlight BDINFO">
                    ...
                    <span class="DTxtSm">{@html building==='Apartment'? 'Wohnung': 'Villa'}</span>
                </div>

                <div class="p-0 bd-highlight BDINFO">
                    <i class="bx bx-building-house"></i> 
                    <span class="DTxtSm">{@html noOfBeds.split(' ')[0]} Betten</span>
                </div>

                <div class="p-0 bd-highlight BDINFO">
                    <i class="bx bx-building-house"></i> 
                    <span class="DTxtSm">{@html noOfBaths.split(' ')[0]} Badezimmer</span>
                </div>
            ...
            <div class="card-text d-flex flex-row bd-highlight Bxinfo">
                ...
                <h3 class="HPrice">{CurrancySBC(parseInt(price *.25))} EUR </h3> 
            </div>

            <div class="card-text d-flex flex-row bd-highlight Bxinfo">

                <div class="p-0 bd-highlight">
                    <a href="tel:{agentNumber}" class="telephone" ><i class="bx bi-telephone"></i> Jetzt anrufen </a> 
                </div>
...
<style>
    ...
    .DetailsBox{ padding: 0px 0px  0px 0px; position: relative;} 
    .BDINFO{ margin: 8px;}
    ...
    .DetailsBox img{ width:90%; height: auto;}
    ...
    @media only screen and (max-width: 768px) {
        ...
        #InnerMyDetails a{ color: #fff; margin: 0px 6px 0px 0px; }
        #InnerMyDetails a.telephone {background-color:#263977;}
        #InnerMyDetails a.whatsapp {background-color:#22D672;}
