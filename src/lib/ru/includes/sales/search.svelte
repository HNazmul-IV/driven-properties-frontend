<script>
    ...
    let searchKey = pgname ? pgname : pageSlug[4];
    ...
    let protype=`${ searchKey[1] ? capitalizeFirstLetter(searchKey[1]) : 'Art der Immobilie' }`;
    let minPrice='';
    let maxPrice='';
    ...
    const FrmSubmit = ()=>{
        ...
        const FrmData = {
            'No_of_Rooms': (bedrooms!=='Bedrooms Type' || bedrooms !=='Schlafzimmertyp')? parseInt(bedrooms.split(' ')[0]):'any',
            ...
            'Unit_Type': (protype!=='Property Type' || protype !=='Art der Immobilie')?capitalizeFirstLetter(protype):'any',
            'Property_Title':capitalizeFirstLetter(protitle),            
            'FrmSearchKey':searchKey
        };
        // console.log(
        //     `/de/properties-for-${prostatus.split(' ')[1].toLowerCase()}/       
        //     ${protitle!==''?protitle.replace(/\s+/g, "-").toLowerCase():'any'}/
        //     ${protype!=='Art der Immobilie'?protype.toLowerCase():'any'}/
        //     ${bedrooms!=='Schlafzimmertyp'? parseInt(bedrooms.split(' ')[0]):'any'}/
        //     ${range2[0]}-${range2[1]}`
        // )
        
        goto(`/de/properties-for-${prostatus.split(' ')[1].toLowerCase()}/${protitle!==''?protitle.replace(/\s+/g, "-").toLowerCase():'any'}/${protype!=='Art der Immobilie'?protype.toLowerCase():'any'}/${bedrooms!=='Schlafzimmertyp'? parseInt(bedrooms.split(' ')[0]):'any'}/${range2[0]}-${range2[1]}`);
            ...
            <div class="row BxInfo-row">
                <div class="col-lg-2 mb-2 col-md-6">
                    <div class="input-group">
                        <select id="FSR" name="prostatus" class="form-select fsr"  on:change={Mysale}>
                            <option selected> Verkauf</option>
                ...
                <div class="col-lg-10  col-md-6">
                    <div class="input-group" id="MyCommunityLists">
                        <Select
                            ...
                            placeholder="Suchort oder Projekt"
                            showIndicator=true
                            isClearable=true
                        ...
                        <button class="btn btn-primary btnfrm" id="mySearch" >suchen</button>
                    </div>
                </div>
