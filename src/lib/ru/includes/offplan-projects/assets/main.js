    ...
    var sw = function() {
        ...
        projectMap = function(locations) {
            ...
            function initialize() {
                ...
                var myOptions = {
                    center: new google.maps.LatLng(25.085975021340353, 55.3868048552284),
                    zoom: 10,
                    styles: grayStyle,
        ...
        country =  {
            ...
            createInput: function(input, cl) {
                ...
                elParent.insertBefore(countryInput, elParent.firstChild);
				ths.isMask = true;
				if(input.dataset.prefer) {
					defaultPrefer = ths.setDefaultCountry(input.dataset.prefer);
					countryInput.setAttribute("data-flag",defaultPrefer.flag);
					countryInput.setAttribute("data-dial",defaultPrefer.dial);
					ths.setFlag(countryFlag,defaultPrefer.flag);
					ths.setInputValue(input,defaultPrefer.dial);
					
					var mask = ths.getMaskFromDial(ths.countries,defaultPrefer.dial);
					if(ths.isMask) {
						ths.setMaskInput(input,"+" + defaultPrefer.dial,mask);
					}
					if(!mask) {
						var newNumber = input.value;
						newNumber = ths.removeMaskCharacter(newNumber);
            ...
            setInputValue: function(el,dial) {
				...
				if(ths.isMask) {
					ths.setMaskInput(el,"+" + dial,mask);
				}
				if(!mask) {
						newNumber = ths.removeMaskCharacter(newNumber);
						//elInput.value = newNumber;
				}
				...
				$(el).closest("FORM").find("[name=phonecode]").val(newDialCode);
				var newPhoneNumber =newNumber.substring(newDialCode.length);
				$(el).closest("FORM").find("[name=phonenumber]").val(newPhoneNumber);

            },
			removeMaskCharacter: function(val) {
				var newVal ="";
					var specARR = ["_","(",")"," "];
					for (var i = 0; i < val.length; i++) {
						if(specARR.indexOf(val[i]) === -1) {
							newVal += val[i];
						}
					}
			...
			setDefaultCountry: function(prefer) {
				var ths = this;
            ...
            setListItem: function(el, cl) {
                ...
                var element = el.closest("." + cl.element),
                    ...
                    element.setAttribute("data-flag",flag);
					element.setAttribute("data-dial",dial);
                    ths.setFlag(elFlag,flag);
                    ths.setInputValue(elInput,dial);
					
					
					if(!mask) {
						var newNumber = elInput.value;
						newNumber = ths.removeMaskCharacter(newNumber);
						//elInput.value = newNumber;
					}
                    elInput.focus();
		...
		phoneCountry = function() {
			...
			phones.forEach(function(phone) {
				...
				$(phone).val("971");
				var pnumber = $(phone).closest("form").find("[name=phonenumber]")[0];
				setMask(pnumber,maskDATA,"tr");
				
			});
			
			function setMask(el, data, flag) {
				if(data[flag]) {
					$(el).inputmask(data[flag]);
				} else {
					$(el).inputmask('remove');
				}
			}
			...
			$(phones).on("countrychange", function(e,countryData) {
				...
				$(input).val(code);
				var elNumber = $(input).closest("form").find("[name=phonenumber]")[0];
				setMask(elNumber,maskDATA,countryData.iso2);
				elNumber.value="";
