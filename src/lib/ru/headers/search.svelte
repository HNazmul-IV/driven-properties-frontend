<script>
    ...
    afterUpdate(()=> {
        ...
        if($page.url.pathname === '/' || $page.url.pathname === '/de'){
            isHome = true;
        }
    })
    ...
    const handleFormClickBtn = (e) =>{
        console.log('search.svelte = ',FrmData);
        ...
        <form on:submit|preventDefault={handleFormClickBtn}>
            <div class="row BxInfo-row">
                ...
                <div class="col-lg-4 col-md-6 mt-2" id="DrvnSlider">
                    {#if isStatus === 'rent'}
                        <RangeSlider step={7} bind:values={range3} min={0} max={3000000*.27} pips  on:change={frmOnChanges} />
                        <!-- svelte-ignore a11y-label-has-associated-control -->
                        <div class="range-slider-input-container">
                            <div class="price-block"><label>prix minimum</label><span class="price-symbol">USD</span><input type="text" class="price minprice" name="minPrice" bind:value="{range3[0]}"/></div> 
                            <div class="dash-sign">-</div>
                            <!-- svelte-ignore a11y-label-has-associated-control -->
                            <div class="price-block"><label>prix maximum</label><span class="price-symbol">USD</span><input type="text" class="price maxprice" name="maxPrice" bind:value="{range3[1]}"/></div>
                        </div>
                    {:else}
                        <RangeSlider bind:values={range2} min={0} max={150000000*.27} pips  on:change={frmOnChanges} />
                        <!-- svelte-ignore a11y-label-has-associated-control -->
                        <div class="range-slider-input-container">
                            <div class="price-block"><label>prix minimum</label><span class="price-symbol">USD</span><input type="text" class="price minprice" name="minPrice" bind:value="{range2[0]}"/></div> 
                            <div class="dash-sign">-</div>
                            <!-- svelte-ignore a11y-label-has-associated-control -->
                            <div class="price-block"><label>prix maximum</label><span class="price-symbol">USD</span><input type="text" class="price maxprice" name="maxPrice" bind:value="{range2[1]}"/></div>
                        </div>
                    {/if}
