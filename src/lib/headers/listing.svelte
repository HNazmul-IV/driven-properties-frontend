...
<div class="row no-gutters" id="InnerMyDetails">
    ...
    <div class="col-lg-5 col-md-6 DetailsBox">
        <Swiper lazy={true} rewind={true} navigation={true} pagination={{ dynamicBullets: true, }} modules={[Lazy,Pagination,Navigation]}  class="mySwiper">
            ...
            -->
            <SwiperSlide>
                <a href="/{(Emirate).toLowerCase()}/{(building).toLowerCase()}-for-{(PropertyStatus).toLowerCase()}/{((Community).toLowerCase()).replaceAll(' ','-')}/{((propertyTitle).toLowerCase()).replaceAll(' ','-')}/{(PropertyRefNo).toLowerCase()}" id="MyLksImg" >
                    <img data-src="{images}" src="{LazyLoadImgURL}" class="swiper-lazy img-fluid" alt={title}/>
                </a>
            </SwiperSlide>
        </Swiper>
    </div>
    ...
    <div class="col-lg-7  col-md-6">
        <div class="card-body">
            ...
            <h5 class="card-title HPrice">
                <a href="/{(Emirate).toLowerCase()}/{(building).toLowerCase()}-for-{(PropertyStatus).toLowerCase()}/{((Community).toLowerCase()).replaceAll(' ','-')}/{((propertyTitle).toLowerCase()).replaceAll(' ','-')}/{(PropertyRefNo).toLowerCase()}" class="MyLkstxt">
                    {@html title }
                </a>
            </h5>
...
<style>
    ...
    .BDINFO{ margin: 8px;}
    #InnerMyDetails { margin-bottom: 2rem !important; padding-bottom: 1.5rem;  }
    ...
    @media only screen and (max-width: 768px) {
        ...
        #InnerMyDetails span.DTxtlm {font-weight: 400 !important; font-size: 1rem;}
