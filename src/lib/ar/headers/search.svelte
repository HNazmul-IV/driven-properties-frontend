<script>
    ...
    let bedrooms='نوع غرف النوم';
    let protype='نوع الملكية';
            ...
            <div class="row BxInfo-row">
                    ...
                    <div class={!isHome? "col-lg-10  col-md-6 community-search": "col-lg-12  col-md-6"} >
                        <div class="input-group" id="MyCommunityLists1">
                            <Select
                                ...
                                placeholder="المدينة أو المنطقة أو اسم البناء"
                                showIndicator=true
                                isClearable=true
                ...
                <div class="col-lg-4  col-md-6 mt-2">
                    <select id="FSRONE" name="protype" class="form-select frsone" on:change={frmOnChanges} bind:value="{protype}">
                        <option selected>نوع الملكية</option>  
                        {#each PropertyType as propertytype, pt }
                            <option value="{propertytype.pttitle_ar}"> {propertytype.pttitle_ar} </option>
                        {/each}
                    </select>
                </div>
                ...
                <div class="col-lg-4  col-md-6 mt-2">
                    <select id="FSRONE" name="bedrooms" class="form-select frsone" on:change={frmOnChanges} bind:value="{bedrooms}">
                        <option disabled>نوع غرف النوم</option>
                        {#each BedRoomType as bdroomtype, bd }
                            <option value="{bdroomtype.bdtitle_ar}">{bdroomtype.bdtitle_ar}</option>
                        {/each}
                    </select>
                </div>
                ...
                <div class="col-lg-4 col-md-6 mt-2" id="DrvnSlider">
                    {#if isStatus === 'rent'}
                        ...
                        <div class="range-slider-input-container">
                            <div class="price-block"><label>سعر دقيقة</label><span class="price-symbol">د.إ (AED)</span><input type="text" class="price minprice" name="minPrice" bind:value="{range3[0]}"/></div> 
                            <div class="dash-sign">-</div>
                            <!-- svelte-ignore a11y-label-has-associated-control -->
                            <div class="price-block"><label>أقصى سعر</label><span class="price-symbol">د.إ (AED)</span><input type="text" class="price maxprice" name="maxPrice" bind:value="{range3[1]}"/></div>
                        </div>
                    {:else}
                        <RangeSlider bind:values={range2} min={0} max={150000000} pips  on:change={frmOnChanges} />
                        <!-- svelte-ignore a11y-label-has-associated-control -->
                        <div class="range-slider-input-container">
                            <div class="price-block"><label>سعر دقيقة</label><span class="price-symbol">د.إ (AED)</span><input type="text" class="price minprice" name="minPrice" bind:value="{range2[0]}"/></div> 
                            <div class="dash-sign">-</div>
                            <!-- svelte-ignore a11y-label-has-associated-control -->
                            <div class="price-block"><label>أقصى سعر</label><span class="price-symbol">د.إ (AED)</span><input type="text" class="price maxprice" name="maxPrice" bind:value="{range2[1]}"/></div>
                        </div>
                    {/if}
...
<style>
    ...
    #FSRONE {
        ...
        padding: 10px 20px;
        border-radius: 0;
        background-position: left 1.13rem center !important;
    }
