<script>
    ...
    let selected ={id:227, code:"+971", country:"United Arab Emirates"};

</script>
<div class="container aos-init aos-animate mb-5 mt-1" data-aos="fade-up">
    ...
    <div class="row" id="withInContent">
        ...
        <div class="col-12">
            ...
            <h5>Sorry, we didn't find anything matching your search.</h5>
                            ...
                            <!--select data-placeholder="Choose Communities..." aria-label="Default select example" class="form-select chosen-select">
                                ...
                                {#each Country as Countrylist (Countrylist.id)}
                                    {#if Countrylist.code === '+971'}
                                        <option value="{Countrylist.code}" selected>{@html Countrylist.code} {@html Countrylist.country}</option>
                                    {:else}
                                        <option value="{Countrylist.code}">{@html Countrylist.code} {@html Countrylist.country}</option>   
                                    {/if}
                                {/each}
                            </select-->
                ...
                <div class="col-12 mt-3 mb-3">
                    <select id="FSRONE" name="bedrooms" class="form-select form-control frsone" bind:value="{bedrooms}">
                        ...
                        {#each BedRoomType as bdroomtype, bd }
                            <option value="{bdroomtype.bdtitle}">{@html bdroomtype.bdtitle}</option>
                        {/each}
                    </select>
                </div>
